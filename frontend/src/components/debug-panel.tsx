...
export function DebugPanel() {
    ...
    useEffect(() => {
        ...
        const savedPos = localStorage.getItem('debugPanelPos');
        if (savedPos) {
            try {
    ...
    const handleDragStop = (_e: unknown, data: { x: number; y: number }) => {
        ...
        setPosition(newPos);
        localStorage.setItem('debugPanelPos', JSON.stringify(newPos));
    };
    ...
    return (
        ...
        >
            <Card
                ref={nodeRef2}
                className={`fixed bottom-6 left-12 z-[100] shadow-2xl border-secondary/20 transition-[width,height] duration-300 overflow-hidden backdrop-blur-md bg-background/95 ${isMinimized ? 'w-64 h-14' : 'w-96 h-[32rem]'}`}
            >
                <CardHeader className="drag-handle flex flex-row items-center justify-between py-3 px-4 bg-muted/40 border-b cursor-move">
